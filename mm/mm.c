...
static void mv_mc_to_hs(u32 master_hartid) {
  ...
  printk("master core %d mmu enabled!\n", master_hartid);
  RESTORE_MC_MC_TO_HS_STACK
}

static void mv_sc_to_hs(void) { RESTORE_MV_SC_TO_HS_STACK }
...
void mm_slave(void) {
  ...
  if (!slave_core_jumper) {
    // actually when this truly happened,it will never show it.
    panic("slave core jumper is not set!");
  }
  slave_core_jumper();
  printk("slave core %d mmu enabled!\n", SMP_GET_HARTID());
  RESTORE_MM_SLAVE_STACK
}
