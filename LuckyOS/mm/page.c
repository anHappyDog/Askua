...
extern size_t __kernel_end__[];
error_t mm_paging(size_t mem_base, size_t mem_size) {
  size_t ekernel = (size_t)__kernel_end__ & ~VIRTUAL_KERNEL_BASE;
  size_t free_npage = (mem_base + mem_size - ekernel) >> PAGE_SHIFT;
  ...
  for (size_t i = 0; i < mpging.npages; ++i) {
    if (i * PAGE_SIZE + mem_base < ekernel) {
  ...
  mpging.pb_ops->alloc_init(mpging.pages + (mpging.npages - free_npage),
                            free_npage);
  return E_OK;
}
