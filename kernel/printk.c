...
void print_tf(const trapframe_t *tf)
{
	printk("trapframe at " HEX_BITS "\n"
		   ...
		   " sscratch: " HEX_BITS "\n",
		   tf, tf->regs[1], tf->regs[3], tf->regs[4], tf->regs[10], tf->regs[11], tf->regs[12], tf->regs[13], tf->regs[14], tf->regs[15], tf->regs[16], tf->regs[17], tf->sepc, tf->sstatus, tf->scause, tf->stval, tf->sscratch);
}
