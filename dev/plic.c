...
void plic_init(size_t base, size_t size) {
  ...
  *(volatile uint32_t *)(base + SIFIVE_INT_ENABLE_BASE +
                         ...
                         (VIRTIO_MMIO_INT / 32) * 4) |=
      1 << (VIRTIO_MMIO_INT % 32);
  *(volatile uint32_t *)(base + SIFIVE_INT_PRI_THRESHOLD_BASE +
                         0x1000 * (2 * hartid + 1)) = 0;
...
uint32_t plic_claim(size_t base, size_t context) {
  return *(volatile uint32_t *)(base + SIFIVE_INT_PRI_CLAIM_BASE +
                                0x1000 * context);
}

void plic_complete(size_t base, size_t context, uint32_t irq) {
  *(volatile uint32_t *)(base + SIFIVE_INT_PRI_CLAIM_BASE + 0x1000 * context) =
      irq;
}